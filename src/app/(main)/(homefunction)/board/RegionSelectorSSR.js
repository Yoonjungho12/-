 ...
 * - 모바일: 3칸 Flex로 정렬, 폰트 축소 & 한 행에서 높이 동일화
 * - regionSlug / themeName 파라미터에 따라 선택 상태 표시
 * - 지역/테마 전부 하드코딩
 */

// ────────────────────────────────────────────────────
// (1) 지역 목록 (하드코딩, id=12 홈케어 제외)
// ────────────────────────────────────────────────────
const REGIONS = [
  // 상위 지역 (id=1~11)
...
// PC에서는 상위지역 6칸, 하위지역 7칸, 테마 9칸 등을 쪼개기
// ────────────────────────────────────────────────────
...
export default function RegionSelectorSSR({ regionSlug, themeName }) {
  ...
  // 4) themeName 분석 → selectedThemeIds
  ...
  function getTdStyle(isSelected) {
    return {
      ...
      textAlign: "center",
      // PC에서는 table, 모바일에서는 flex -> 그래도 수직정렬 잘 되게
      verticalAlign: "top",
    };
  }
        ...
        */
        @media (max-width: 768px) {
          ...
          table.region-table tr {
            display: flex;
            flex-wrap: wrap;
            align-items: stretch; /* 한 행에서 가장 긴 셀 높이에 맞춤 */
            margin: 0;
            padding: 0;
          }
