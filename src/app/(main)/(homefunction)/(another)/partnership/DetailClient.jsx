...
export default function DetailClient({ row, images, numericId, showBlurDefault }) {
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      if (error || !data?.session) {
        console.warn("🚨 세션 없음 또는 오류 - 자동 로그아웃 처리");
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(
      async (event, newSession) => {
        if (event === "SIGNED_OUT") {
          console.log("🧹 로그아웃 이벤트 감지됨 - 상태 초기화");
          localStorage.removeItem("anon_user_id");
          setSession(null);
          router.refresh();
  ...
  // (C) 조회수
  useEffect(() => {
  if (numericId === undefined || !userId || !numericId || hasCountedView) return;
    (async () => {
      try {
  ...
  // (D) "가고싶다"
  useEffect(() => {
  if (numericId === undefined || !session?.user?.id || !numericId) return;
    supabase
      .from("wantToGo")
      .select("id")
